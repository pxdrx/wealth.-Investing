...
export default function AccountPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-10">
      <Link href="/app" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground mb-8">
            ...
            {(["light", "dark", "system"] as const).map((t) => (
              <button key={t} type="button" onClick={() => setTheme(t)}
                className={`px-4 py-2 rounded-[10px] text-sm font-medium border transition-colors ${
                  theme === t ? "bg-foreground text-background border-foreground" : "bg-background text-foreground border-border hover:bg-muted/50"
                }`}>
      ...
      <section>
        ...
        <div className="rounded-[16px] border border-border bg-card p-6">
          ...
          <button type="button" onClick={handleLogout} className="text-sm font-medium text-destructive hover:underline">
            Sair da conta
          </button>
        </div>
      </section>
